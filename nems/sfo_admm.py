...
class SFO(object):
    ...
    def check_grad(self, small_diff = None):
        ...
        for i in np.random.permutation(list(range(self.N))):
            ...
            for j in np.random.permutation(list(range(self.M))):
                ep[j] = small_diff
    ...
    def update_hessian(self,indx):
        ...
        for hist_i in reversed(list(range(num_hist))):
    ...
    def set_theta(self, theta_new, rho):
        """
        Set the current value of the parameters, manually
        """
        self.theta = self.theta_original_to_flat(theta_new)
        self.theta_proj = np.dot(self.P.T, self.theta)
        self.admm_rho = rho
