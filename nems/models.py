...
from .sfo import SFO as SFO_orig
...
class LNExp(NeuralEncodingModel):
    def __init__(self, stim, rate, filter_dims, minibatch_size=None, frac_train=0.8, num_tents=30, sigmasq=0.2,
        ...
        self.theta_init['b'] = np.zeros((1,))
        self.theta_init['c'] = np.zeros((1,))
...
#        W = np.ones_like(W)
        ...
        u, r = self._rate({'W': W, 'b': b, 'c' :  c}, data['stim'])
...
#        dw = np.zeros_like(W)
    ...
    def metrics(self, data_index):
        ...
        rhat = self._rate(self.theta, self.data[data_index]['stim'])[-1]
    ...
    #    k, n, tau = W.shape
        ...
        theta_current = {'W': self.theta_init['W'].copy(), 'b': self.theta_init['b'].copy(), 'c': self.theta_init['c'].copy()}
