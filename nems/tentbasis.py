...
class Nonlinearity(object):
    ...
    def __init__(self, tent_span, num_tents, num_samples):
        """
...
class Gaussain(Nonlinearity):
    def __init__(self, tent_span, num_tents, num_samples=1000, sigmasq=0.02):
        ...
        self.Phi = np.zeros((num_samples, num_tents))
        for idx, c in enumerate(self.centers):
            Phi[:, idx] = np.exp(-0.5 * (x - centers[j])**2 / sigmasq )
    ...
    def evaluate(x):
        ...
        for idx, c in enumerate(self.centers):
            z = np.exp( -0.5 * (x.ravel() - c)**2 / self.sigmasq )
            zgrad = - (x.ravel() - c) / self.sigmasq

            Phi[:,idx] = z
            PhiGrad[:,idx] = z * zgrad
            if hess:
                PhiGrad2[:,idx] = PhiGrad[:,idx] * zgrad - z / sigmasq
    ...
    # def fit(self, x, y):
        # """
        # Chooses nonlinearity parameters such that f(x) approximates y
        # """
...
class Linear(Nonlinearity):
    ...
    def __init__(self, tent_span, num_tents, num_samples=1000):

        super().__init__(tent_span, num_tents, num_samples)

        # centers of the basis functions
        self.centers = np.linspace(self.tent_span[0], self.tent_span[1], self.num_tents)

        # build basis functions
        self.Phi = np.zeros((self.num_samples, self.num_tents))
        for c in range(num_tents):

            # left leg of the tent
            if c > 0:
                il = np.where(((x >= self.centers[c-1]) & (x <= self.centers[c])))
                self.Phi[il, c] += (x[il] - self.centers[c-1]) / (self.centers[c] - self.centers[c-1])

            # right leg of the tent
            if c < num_tents - 1:
                ir = np.where(((x >= self.centers[c]) & (x <= self.centers[c + 1])))
                self.Phi[ir, c] += (self.centers[c+1] - x[ir]) / (self.centers[c+1] - self.centers[j])
